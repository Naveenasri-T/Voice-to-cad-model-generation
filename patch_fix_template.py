"""
Patch: Fix the _build_blueprint_template in ai_service.py
- Draft.makeText  → Draft.make_text
- Draft.makeLine  inside wall() → safe color via try/except
- All ViewObject color assignments wrapped in try/except
- Section hatching removed (caused "scratched" look)
- Grid spacing doubled to reduce visual noise
- dim() ViewObject color simplified
"""
import sys, re
...
START = "    def _build_blueprint_template(self, command: str) -> str:"
...
if s == -1 or e == -1:
    print(f"ERROR markers not found: s={s} e={e}", file=sys.stderr)
    sys.exit(1)

NEW_METHOD = r'''    def _build_blueprint_template(self, command: str) -> str:
...
new_src = src_lf[:s] + NEW_METHOD + src_lf[e:]

with open(TARGET, "w", encoding="utf-8") as f:
    f.write(new_src)

print(f"Patched {TARGET}")
print(f"  Replacement: {len(NEW_METHOD)} chars")
