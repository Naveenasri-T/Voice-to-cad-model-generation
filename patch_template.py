"""
Standalone script that patches _build_blueprint_template in ai_service.py
with a proper 2BHK architectural floor plan blueprint.
"""
import re, os, sys
...
start_marker = "    def _build_blueprint_template(self, command: str)"
end_marker   = "    def _generate_with_groq"
...
# Replace the block
new_src = src[:start_idx] + NEW_METHOD + "\n" + src[end_idx:]

with open(TARGET, "w", encoding="utf-8") as f:
    f.write(new_src)

print(f"Successfully patched {TARGET}")
print(f"  Old length: {len(src)}")
print(f"  New length: {len(new_src)}")
